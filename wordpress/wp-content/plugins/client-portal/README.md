...
Use the type filter above the log table to focus on specific events (for example, `rest_post` or `portal_erasure_failed`). All filter selections and exports are captured in the audit trail so teams can trace who accessed sensitive records.
