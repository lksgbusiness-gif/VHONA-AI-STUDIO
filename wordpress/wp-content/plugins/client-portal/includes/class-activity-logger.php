...
class ActivityLogger
{
     ...
     *
     * @param array $args Optional overrides. Supports `type`, `paged`, `posts_per_page`,
     *                    `include_meta`, `search`, `after`, and `before`.
     *
     * @return array<mixed>
     */
     ...
     *
     * @param array $args Optional filters that mirror get_entries().
     *
     * @return string
     */
    public function to_csv(array $args = [])
    {
        $rows   = [
            ['ID', 'Type', 'Actor', 'Message', 'Created', 'Context'],
        ];

        $args['posts_per_page'] = isset($args['posts_per_page']) ? (int) $args['posts_per_page'] : 200;

        $result = $this->get_entries($args);
    ...
    private function build_query_args(array $args)
    {
        ...
        if (!empty($args['after'])) {
            if ($after = $this->normalize_date($args['after'])) {
                $date_query[] = [
        ...
        if (!empty($args['before'])) {
            if ($before = $this->normalize_date($args['before'])) {
                $date_query[] = [
                    'column'    => 'post_date_gmt',
                    'before'    => $before,
                    'inclusive' => true,
    ...
    /**
     * Convert human input into a date string recognised by WP_Query.
     *
